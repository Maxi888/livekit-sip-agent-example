  ...
  svc.on('session:new', async(session) => {
    ...
    try {
      ...
      if (outboundFromLiveKit) {
        ...
        // Previously we used query parameters, but let's try just a clean SIP URI with the LiveKit call ID
        // The call ID should be unique and LiveKit should associate it with the caller and callee
        target = [
          {
            type: 'sip',
        ...
        // Add debugging headers - these will be passed to LiveKit
        headers = {
          'X-LiveKit-Call-ID': livekit_call_id,
          'X-Caller': from,
          'X-Callee': dest,
          'X-Session-ID': call_sid,
          'X-URI-Format': 'simple',  // Indicate we're using a simple format
          'Contact': `<sip:${livekit_call_id}@${sipDomain}>`,  // Try to help the SIP routing
          'X-Custom-Route': 'direct'  // Signal to LiveKit to try direct routing
        };
        
        logger.info({sipUri: `sip:${livekit_call_id}@${sipDomain}`, headers}, 'Formatted SIP URI for LiveKit');
      }
